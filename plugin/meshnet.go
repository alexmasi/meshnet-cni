...
func cmdAdd(args *skel.CmdArgs) error {
	...
	peerLinks := map[string][]*mpb.Link{}
	for _, link := range localPod.Links {
		if _, ok := peerLinks[link.PeerPod]; !ok {
			peerLinks[link.PeerPod] = []*mpb.Link{}
		}
		peerLinks[link.PeerPod] = append(peerLinks[link.PeerPod], link)
	}

	log.Infof("Add[%s]: Starting to traverse all links", string(cniArgs.K8S_POD_NAME))
	for peer, links := range peerLinks {
		log.Infof("Add[%s]: retrieving peer pod %s information from meshnet daemon", string(cniArgs.K8S_POD_NAME), peer)
		peerPod, err := meshnetClient.Get(ctx, &mpb.PodQuery{
			Name:   peer,
			KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
		})
		if err != nil {
			log.Errorf("Add[%s]: Failed to retrieve peer pod %s:%s topology", string(cniArgs.K8S_POD_NAME), string(cniArgs.K8S_POD_NAMESPACE), peer)
			return err
		}
		lsr, err := meshnetClient.ListSkipped(ctx, &mpb.ListSkippedRequest{
			Pod:    localPod.Name,
			Peer:   peer,
			KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
		})
		...
		for _, s := range lsr.LinkIds {
			skipped[s] = true
		}
		toSkipC := make(chan int64, len(links))
		g := new(errgroup.Group)
		for _, link := range links {
			peerPod := peerPod
			link := link
			g.Go(func() error {
				skip, err := addLink(ctx, meshnetClient, localPod, peerPod, link, srcIntf, cniArgs, skipped)
				if skip {
					toSkipC <- link.Uid
				}
				return err
		...
		err = g.Wait()
		close(toSkipC)
		bsr := &mpb.BatchSkipRequest{}
		for ts := range toSkipC {
			bsr.Skip = append(bsr.Skip, &mpb.SkipQuery{
				Pod:    localPod.Name,
				Peer:   peerPod.Name,
				KubeNs: string(cniArgs.K8S_POD_NAMESPACE),
				LinkId: ts,
		...
		log.Infof("Add[%s]: executing batch skip for %v links", string(cniArgs.K8S_POD_NAME), len(bsr.Skip))
		if _, err := meshnetClient.BatchSkip(ctx, bsr); err != nil {
			log.Errorf("Failed to BatchSkip: %v", err)
		}
		if err != nil {
			return err
		}
...
func addLink(ctx context.Context, meshnetClient mpb.LocalClient, localPod *mpb.Pod, peerPod *mpb.Pod, link *mpb.Link, srcIntf string, cniArgs k8sArgs, skipped map[int64]bool) (bool, error) {
	...
	if isAlive { // This means we're coming up AFTER our peer so things are pretty easy
		...
		if peerPod.SrcIp == localPod.SrcIp { // This means we're on the same host
			...
			if iExist && pExist { // If both link exist, we don't need to do anything
				...
				isSkipped := skipped[link.Uid]
				log.Infof("Add[%s]: Have we been skipped for %s (link id %d) by our peer %s? %t", string(cniArgs.K8S_POD_NAME), link.LocalIntf, link.Uid, peerPod.Name, isSkipped)
